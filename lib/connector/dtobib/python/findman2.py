...
def startPurge(jsonfilters,context):
	...
	if context.nbProcessedFiles>0:
		...
		sys.stderr.write(str(context.nbProcessedFiles)+" files processed\n")
	if context.nbProcessedDirs>0:
		purgelog(context,str(context.nbProcessedDirs)+" dirs processed")
		sys.stderr.write(str(context.nbProcessedFiles)+" dirs processed")
	
	#purge_sup 
	if context.options.matchfile:
		cmd="purge_sup sendevent objname=OS_PURGE, cdetape=END, inputfile="+context.options.matchfile
		if context.options.verbose:
			print2("\n###\ncalling cmd : ",cmd)
		purgelog(context,"\n###\ncalling cmd : "+cmd)
		...
		while True:
						...
						if context.options.verbose:
							print2(line)
						purgelog(context,line)
		
	if context.options.unmatchfile:
		cmd="purge_sup sendevent objname=OS_SPACE, cdetape=END, inputfile="+context.options.unmatchfile
		if context.options.verbose:
			print2("\n###\ncalling cmd : ",cmd)
		purgelog(context,"\n###\ncalling cmd : "+cmd)
		p = subprocess.Popen(cmd, 
