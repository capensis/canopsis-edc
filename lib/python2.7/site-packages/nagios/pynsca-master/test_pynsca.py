...
class TestNetwork(unittest.TestCase):
    ...
    def test_xor(self):
        ...
        notif.svc_result('linux-ix-subordinate10.build', 'buildbot-start', 0, 'hello!')
...
class TestPacketMethods(unittest.TestCase):
    ...
    def test_encode_service_to_server(self):
        ...
        pkt = self.notif._encode_to_server(iv, timestamp, 0,
                'linux-ix-subordinate10.build', 'buildbot-start', 'hello!')
        self.assertEqual(
    ...
    def test_encode_host_to_server(self):
        ...
        pkt = self.notif._encode_to_server(iv, timestamp, 0,
                'linux-ix-subordinate10.build', '', 'hello!')
        self.assertEqual(
                [ ord(b) for b in exp_pkt ],
                [ ord(b) for b in pkt ])
    ...
    def test_encode_service_to_server_supports_unicode_strings(self):
        ...
        pkt = self.notif._encode_to_server(iv,
                                           timestamp,
                                           0,
                                           u'linux-ix-subordinate10.build',
                                           u'buildbot-start',
                                           u'teste: éçãê')
        self.assertEqual(
            [ord(b) for b in exp_pkt],
